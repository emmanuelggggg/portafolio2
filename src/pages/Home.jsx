import React , {useState } from 'react'
import user from '../assets/img/imgUser/io.jpg'
import iconUser from '../assets/img/icons/user.png'
import iconFlecha from '../assets/img/icons/flecha.png'
import iconFlechaDown from '../assets/img/icons/flechaDown.png'
import iconSkills from '../assets/img/icons/skills.png'
import iconB from '../assets/img/icons/b.png'
import iconReact from '../assets/img/icons/react.png'
import iconHtml from '../assets/img/icons/html.png'
import iconCss from '../assets/img/icons/css.png'
import iconJs from '../assets/img/icons/js.png'
import iconGit from '../assets/img/icons/git.png'
import iconMysql from '../assets/img/icons/mysql.png'
import iconTailwind from '../assets/img/icons/tailwind.png'
import iconFigma from '../assets/img/icons/figma.png'
import iconSass from '../assets/img/icons/sass.png'
import iconGorro from '../assets/img/icons/gorro.png'

const about = "I am currently a software development engineering student. 1 year of experience in frontend. My adventure begins two and a half years ago in college, I have done distance work, social service and professional practices in autonomous places of government La Paz B.C.S.(Mexico). I am a calm and confident person, curious by nature, I work to improve the life of my wife and son." ;

const AccordionItem = ({ title, content, imageOpen , imageClosed, iconOpen, iconClosed}) => {
  const [isOpen, setIsOpen] = useState(false);

  return (
    <div className={`accordion-item ${isOpen ? 'active' : ''}`}>
      <div className="accordion-header" onClick={() => setIsOpen(!isOpen)}>
        <div className='space'>

          <div className='spaceS'>
            <div className='contentImg'>
              <img src={isOpen ? imageOpen : imageClosed} alt={`Icono de ${title}`} className="iconUser" />
            </div>
            {title}
          </div>

          <div>
            <img src={isOpen ? iconOpen : iconClosed} alt={`Icono de ${title}`} className="iconFlecha"/>
          </div>

        </div>
      </div>
      {isOpen && <div className="accordion-content">{content}</div>}
    </div>
  );
};


const Home = () => { 
  return (
    <div className='content1'>
      {/*nombre*/}
      <div className='fl'>

        <div>
          {/*prop aqui*/}
          <p className='name'>Jesus Emmanuel C.</p>
        </div>

        <div className='perfilContent'>
          <img  className='fotoPerfilContent' src={user} alt="" />
          
          {/*icons social*/}
          <p>Software Development Engineer</p>
          <span>Many1997XD@gmail.com</span>
          <div className='icon-social'>
              <div>
                <a href="https://github.com/emmanuelggggg" target="_blank" rel="noreferrer">
                  <svg xmlns="http://www.w3.org/2000/svg" width="26" height="16" fill="white" class="bi bi-github" viewBox="0 0 16 16">
                    <path d="M8 0C3.58 0 0 3.58 0 8c0 3.54 2.29 6.53 5.47 7.59.4.07.55-.17.55-.38 0-.19-.01-.82-.01-1.49-2.01.37-2.53-.49-2.69-.94-.09-.23-.48-.94-.82-1.13-.28-.15-.68-.52-.01-.53.63-.01 1.08.58 1.23.82.72 1.21 1.87.87 2.33.66.07-.52.28-.87.51-1.07-1.78-.2-3.64-.89-3.64-3.95 0-.87.31-1.59.82-2.15-.08-.2-.36-1.02.08-2.12 0 0 .67-.21 2.2.82.64-.18 1.32-.27 2-.27s1.36.09 2 .27c1.53-1.04 2.2-.82 2.2-.82.44 1.1.16 1.92.08 2.12.51.56.82 1.27.82 2.15 0 3.07-1.87 3.75-3.65 3.95.29.25.54.73.54 1.48 0 1.07-.01 1.93-.01 2.2 0 .21.15.46.55.38A8.01 8.01 0 0 0 16 8c0-4.42-3.58-8-8-8"/>
                  </svg>
                </a>
              </div>
              <div>
                <a href="https://twitter.com/JesusEm50037139" target="_blank" rel="noreferrer">
                  <svg xmlns="http://www.w3.org/2000/svg" width="26" height="16" fill="white" class="bi bi-twitter-x" viewBox="0 0 16 16">
                    <path d="M12.6.75h2.454l-5.36 6.142L16 15.25h-4.937l-3.867-5.07-4.425 5.07H.316l5.733-6.57L0 .75h5.063l3.495 4.633L12.601.75Zm-.86 13.028h1.36L4.323 2.145H2.865z"/>
                  </svg>
                </a>
              </div>
              <div>
                <a href="https://www.linkedin.com/in/jesus-emmanuel-carballo-caballero-ab1a39269/" target='_blank' rel="noreferrer">
                  <svg xmlns="http://www.w3.org/2000/svg" width="26" height="16" fill="white" class="bi bi-linkedin" viewBox="0 0 16 16">
                    <path d="M0 1.146C0 .513.526 0 1.175 0h13.65C15.474 0 16 .513 16 1.146v13.708c0 .633-.526 1.146-1.175 1.146H1.175C.526 16 0 15.487 0 14.854zm4.943 12.248V6.169H2.542v7.225zm-1.2-8.212c.837 0 1.358-.554 1.358-1.248-.015-.709-.52-1.248-1.342-1.248S2.4 3.226 2.4 3.934c0 .694.521 1.248 1.327 1.248zm4.908 8.212V9.359c0-.216.016-.432.08-.586.173-.431.568-.878 1.232-.878.869 0 1.216.662 1.216 1.634v3.865h2.401V9.25c0-2.22-1.184-3.252-2.764-3.252-1.274 0-1.845.7-2.165 1.193v.025h-.016l.016-.025V6.169h-2.4c.03.678 0 7.225 0 7.225z"/>
                  </svg>
                </a>
              </div>
          </div>
        
        </div>

        <div className="accordionPrincipal">
          <div className="accordion">
            <AccordionItem              
              title="About"
              content={<p>{about}</p>}
              imageOpen={iconUser}
              imageClosed={iconUser}
              iconOpen={iconFlechaDown}
              iconClosed={iconFlecha}
            />
            <AccordionItem              
              title="Skills"
              content={
                <div className="icons-skills">     
                  <div>
                    <img src={iconB} alt="" />              
                  </div>
                  <div>
                    <img src={iconTailwind} alt="" />              
                  </div>
                  <div>
                    <img src={iconFigma} alt="" />              
                  </div>
                  <div>
                    <img src={iconReact} alt="" />              
                  </div>
                  <div>
                    <img src={iconHtml} alt="" />              
                  </div>
                  <div>
                    <img src={iconCss} alt="" />              
                  </div>
                  <div>
                    <img src={iconSass} alt="" />              
                  </div>
                  <div>
                    <img src={iconJs} alt="" />              
                  </div>
                  <div>
                    <img src={iconGit} alt="" />              
                  </div>
                  <div>
                    <img src={iconMysql} alt="" />              
                  </div>
                </div>
            }
              imageOpen={iconSkills}
              imageClosed={iconSkills}
              iconOpen={iconFlechaDown}
              iconClosed={iconFlecha}
            />
            
            <AccordionItem              
              title="Education"
              content={ <p>Autonomous University of Baja California Sur (UABCS) </p>}
              imageOpen={iconGorro}
              imageClosed={iconGorro}
              iconOpen={iconFlechaDown}
              iconClosed={iconFlecha}
            />

          </div>
        </div>

      </div>
    </div> 
  )
}

export default Home